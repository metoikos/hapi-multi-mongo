    ...
    async register(server, options) {
        ...
        const mongodbUriParser = (uri, options) => {
            return new Promise((resolve, reject) => {
                try {
                    const ops = parseOptions(uri, undefined, options)
                    return resolve(ops)
                } catch (err) {
                    return reject(err)
                }
        ...
        const connect = async (uri) => {
            ...
            if (!connectionObj.hasOwnProperty('name')) {
                ...
                Assert(uriMatch.dbName, 'You have to provide a name for uri or have to connect to ' +
                    'directly to specific database.');

                // set connection objects name
                connectionObj.name = uriMatch.dbName;
            }

            const client = new MongoClient(connectionObj.uri, connectionObj.options);
