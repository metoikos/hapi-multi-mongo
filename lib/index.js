...
exports.register = function (server, options, next) {
    ...
    Promise.map(uris, (uri) => {
        ...
        if (!new RegExp(uriPattern).exec(connectionObj.uri)) {
            const str = 'Invalid mongo connection uri ' + uri;
            server.log(['hapi-multi-mongo', 'error'], str);
            return next(str);
        }
    ...
    server.on('stop', () => {
        Promise.each(Object.keys(connections), (name) => {
            ...
            server.log(['hapi-multi-mongo', 'error'], 'An error occurred while closing connections: ' + err);
        });
    });
};
